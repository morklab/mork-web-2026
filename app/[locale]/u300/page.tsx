...
export default function U300Page() {
  ...
  // Asegúrate de que el ID (111C) sea el correcto.
  const fourVenuesScript = `<script src="https://www.fourvenues.com/assets/iframe/u300/111C"></script>`
  ...
  return (
    <main className="min-h-screen bg-black text-zinc-500 font-mono relative overflow-hidden selection:bg-red-900 selection:text-white">
      
      {/* --- FILTRO SVG (INVISIBLE) --- */}
      ...
      <div className="relative z-10 whole-page-glitch-container min-h-screen">
          ...
          <div className="flex flex-col items-center justify-center min-h-screen px-6 py-20">
            ...
            {/* LOGO DE FONDO (Reemplaza al H1) */}
            <div className="mb-24 mix-blend-overlay opacity-20 select-none pointer-events-none">
                <Image 
                    src="/u300tran.png" // 👈 TU LOGO AQUÍ
                    alt="U300 Background"
                    width={600} // Tamaño grande para el fondo
                    height={250}
                    className="object-contain w-[80vw] md:w-[600px]" // Responsive
                />
            </div>

            {/* BLOQUE DEL EVENTO */}
            <div className="w-full max-w-5xl border-t border-zinc-800">
                <div className="group flex flex-col lg:flex-row justify-between items-center py-12 border-b border-zinc-800 hover:bg-black/40 transition-colors relative overflow-hidden px-4">
                    ...
                    <div className="flex flex-col gap-1 w-full lg:w-1/4 mb-8 lg:mb-0">
                        <span className="text-xs tracking-[0.3em] font-bold text-red-600 animate-pulse">
                            {t('date')}
                        </span>
                    </div>
                    ...
                    <div className="flex flex-1 flex-col items-start lg:items-end justify-center w-full lg:w-auto gap-2">
                        ...
                        <span className="text-4xl md:text-6xl lg:text-7xl uppercase font-black tracking-tighter whitespace-nowrap frequency-text will-change-transform leading-none">
                            Adriana Lopez
                        </span>
                    </div>

                    {/* BOTÓN TICKETS */}
                    <div className="mt-10 lg:mt-0 lg:ml-12 w-full lg:w-auto lg:opacity-0 lg:group-hover:opacity-100 lg:translate-x-4 lg:group-hover:translate-x-0 transition-all duration-300">
                        <button 
                           onClick={() => setIsTicketOpen(true)}
                           className="block w-full lg:w-auto text-center text-[10px] uppercase tracking-[0.2em] bg-transparent border border-white text-white font-bold px-8 py-4 hover:bg-red-600 hover:border-red-600 hover:text-white transition-all cursor-pointer"
                        >
                            {t('cta')}
                        </button>
                    </div>
      ...
      {isTicketOpen && (
        <div className="fixed inset-0 z-[9999] bg-black animate-in fade-in duration-300 flex items-start justify-center overflow-y-auto">
            ...
            srcDoc={`
                ...
                <body>
                    <div class="wrapper">${fourVenuesScript}</div>
                </body>
                </html>
            `}
            />
